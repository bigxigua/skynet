  ...
  <div class="svg-double">
    <svg :width="400*victoryRoundLen" :height="(roundLength[1] + failRounds[1] + 2)*75"
      ...
      <g>
        ...
        <g v-for="(item, index) in doubleLists[0]"
           @click="editHandle(item.vss[0])"
           :transform="getPosition(item, index)">
          ...
          <g v-for="(player, pos) in sortByIndex(item.node_vss)"
             @drop="dropHandle($event, player)" @dragover="dragoverHandle"
             :transform="'translate(0 ' + (pos*24) + ')'"
             class="sb_sub"
             v-hover="'svg_rect_hover'">
            <rect  x="62" y="68" width="200" height="25" style="fill: rgb(52, 72, 96) ; opacity: 1;" :class="'rect1' + player.team_name"></rect>
            <image x="67" y="71" height="20" width="30" xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="player.team_logo || 'http://djq-img.dianjingquan.cn/country/0.jpg'"></image>
            <text  x="102" y="85" font-size="13" style="fill: rgb(184, 186, 188);">{{ player.team_name || '暂无' }}</text>
            <text  x="232" y="85" font-size="13" style="fill: rgb(184, 186, 188);">{{ player.score || 0 }}</text>
      ...
      <g :transform="'translate(0 '+ getHeight(victoryRoundLen) +')'">
        ...
        <g v-for="(item, index) in doubleLists[1]"
           @click="editHandle(item.vss[0])"
           :transform="getPositionFail(item, index)">
  ...
  export default {
    ...
    methods:{
      ...
      dropHandle: function (e, item) {
        e.preventDefault();
        if(e.currentTarget.children[2].innerHTML !== '暂无' || item.round > 1) return;
