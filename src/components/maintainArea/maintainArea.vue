<template>
  <div class="warteam-container">
    ...
    <el-button type="success"
               ...
               @click="searchArea">搜索区服</el-button>
    <el-button
              type="success"
              style="margin-left: 20px"
              @click="showEditBox = true, isUpdate = false, mapName = ''">新增区服</el-button>
    <div class="details_cover" v-if="showEditBox">
      <div class="details">
        <h1>{{ isUpdate ? '编辑' : '新增' }}区服</h1>
        <div class="content">
          <div class="col row">
            <p>当前游戏</p>
            <p>{{ getGameName() }}</p>
  ...
  export default {
    ...
    methods: {
      ...
      getGameMapLists:function (game_id) {
        this.$http.get('/v1/match/manage/gameRegionList', {
          params: {
            game_id: game_id,
            role: 1,
          ...
          try{
            this.gameArea = data.data.data.list;
      ...
      deleteAreaApi: function (mapId) {
        return this.$http.delete('/v1/match/manage/deleteGameRegions?region_id=' + mapId)
      },
      addMapApi: function (query) {
        return this.$http.post('/v1/match/manage/addGameRegions' + query);
      },
      //新增地图
      addMap: function () {
        if(!this.gameName) {
        ...
        let regionName = this.isUpdate ? 'name' : 'region_name';
        let query = '?' + regionName + '=' + this.mapName + '&game_id=' + this.gameName;
        let requestHandle = this.isUpdate ? this.updateMapApi : this.addMapApi;
        this.isUpdate ? (query+='&region_id=' + this.regionId) : {};
      ...
      updateMapApi: function (query) {
        return this.$http.post('/v1/match/manage/updateGameRegions' + query)
      },
      ...
      deleteMap: function (item) {
        this.$confirm('确定删除该地图吗?', '提示', {
          ...
          this.deleteAreaApi(item.regionId).then(data => {
